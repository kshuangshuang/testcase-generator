...
要求：
1. **全面覆盖**：仔细分析需求文档，逐句阅读，识别所有功能点、场景和业务流程，确保每个功能点都有对应的测试用例
2. **不限制数量**：根据需求的复杂程度生成测试用例，简单功能可能5-10条，复杂功能可能20-50条甚至更多，不要人为限制在固定数量
3. **场景完整**：覆盖正常流程、基本异常流程、边界情况
4. **完整输出**：必须生成所有识别到的测试用例，不要遗漏任何功能点
5. 每个用例必须包含：用例ID、用例标题、前置条件、测试步骤、预期结果、优先级
6. **用例ID格式**：使用阿拉伯数字从1开始递增（1, 2, 3...），不要使用 TC-001 这种格式
7. 输出为 Markdown 表格格式
8. 所有内容使用中文
9. **禁止免责声明**：不要在表格前后添加任何说明、提示、免责声明或补充说明（如"以上用例仅覆盖了部分功能点"等），只输出纯表格
...
要求：
1. **全面覆盖**：仔细分析需求文档，使用等价类划分、边界值分析、决策表等方法，逐句识别所有测试点
2. **多维测试**：包含功能测试、边界值测试、异常场景测试、接口测试等
3. **不限制数量**：根据需求复杂程度生成，不要人为限制数量，确保测试覆盖完整，可能需要30-80条甚至更多
4. **完整输出**：必须生成所有识别到的测试用例，不要遗漏任何测试点
5. 每个用例必须包含：用例ID、用例标题、测试类型、前置条件、测试步骤、测试数据、预期结果、优先级
6. **用例ID格式**：使用阿拉伯数字从1开始递增（1, 2, 3...），不要使用 TC-001 这种格式
7. 输出为 Markdown 表格格式
8. 所有内容使用中文
9. **禁止免责声明**：不要在表格前后添加任何说明、提示、免责声明或补充说明，只输出纯表格
...
3. **不限制数量**：根据需求复杂程度和风险评估，生成完整的测试用例集，可能需要50-100条甚至更多
4. **完整输出**：必须生成所有识别到的测试用例，不要遗漏任何测试点
5. 每个用例必须包含：用例ID、用例标题、测试类型、前置条件、测试步骤、测试数据、预期结果、优先级、状态、备注
6. **用例ID格式**：使用阿拉伯数字从1开始递增（1, 2, 3...），不要使用 TC-001 这种格式
7. 输出为 Markdown 表格格式
8. 在开头提供测试分析总结（仅此部分可以有文字说明）
9. 所有内容使用中文
10. **禁止免责声明**：测试用例表格后不要添加任何免责声明或补充说明（如"以上用例仅覆盖了部分"等），表格必须是完整的
