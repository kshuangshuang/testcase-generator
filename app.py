...
def generate_test_cases(content, test_type, api_key):
    ...
    if test_type == "功能测试":
        prompt_instruction = """
你是一个专业的测试工程师。请基于以下需求文档，生成功能测试用例。

要求：
1. 覆盖正常流程和基本异常场景
2. 每个用例必须包含：用例ID、用例标题、前置条件、测试步骤、预期结果、优先级
3. 输出为 Markdown 表格格式
4. 所有内容使用中文
...
要求：
1. 包含功能测试、边界值测试、异常场景测试
2. 使用等价类划分、边界值分析等方法设计测试用例
...
要求：
1. 包含功能、边界、异常、性能、安全等多维度测试
2. 识别需求中的测试点和风险评估
3. 每个用例必须包含：用例ID、用例标题、测试类型、前置条件、测试步骤、测试数据、预期结果、优先级、状态、备注
4. 输出为 Markdown 表格格式
5. 在开头提供测试分析总结
6. 所有内容使用中文
